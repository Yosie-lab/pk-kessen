...
- 依存インストールは不要（`package.json` に依存なし）。起動は `## Commands` の `npm start`（内部的に `python3 -m http.server 5180 --bind 0.0.0.0`）を使う。`python3` は環境に既にある。
- サーバは `0.0.0.0:5180` にバインドするので、動作確認は `http://127.0.0.1:5180/` を開く（`file://` は ES モジュールが壊れるので不可）。
- 自動テストは無い。検証はブラウザで実際にプレイして happy path（キックオフ→シュート→セーブ→結果）を確認する（`## Verification checklist` 参照）。
- サーバは長時間走らせるので tmux セッションで起動しておくとよい（例: セッション名 `pk-dev-server`）。
