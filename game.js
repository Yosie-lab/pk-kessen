...
const I18N = {
  ja: {
    ...
    trialFeat2: "・マイチーム機能で全20ヶ国の代表チームから自由に選択可能",
  ...
  en: {
    ...
    trialFeat2: "・Choose your favorite team from 20 national teams",
  ...
  es: {
    ...
    trialFeat2: "・Selecciona tu equipo favorito entre 20 selecciones",
  ...
  ko: {
    ...
    trialFeat2: "・전 20개국 대표팀 중 자유롭게 선택 가능",
    ...
    whistle: "휘슬!",
    runup: "런업! 킥하는 순간 골대를 클릭",
    runupSave: "상대 런업! 킥하는 순간 골대를 클릭하여 다이빙",
    diveNow: "지금이다! 골대를 클릭하여 다이빙",
    shootNow: "지금이다! 골대를 클릭",
    dive: "다이빙!",
    reactionLate: "반응이 늦었다…",
    shoot: "슛!",
    timingLate: "타이밍이 늦었다…",
    missedFrame: "골대를 벗어났다!",
    goalYou: "골ーーー!!",
    goalCpu: "골을 허용했다…",
    savedYou: "막아냈다〜!",
    savedCpu: "키퍼에게 막혔다〜",
    postHitOut: "에 맞고 벗어났다!",
  ...
  fr: {
    ...
    trialFeat2: "・Choisissez votre équipe parmi 20 sélections nationales",
    ...
    runup: "Course ! Touchez le but lors du tir",
    runupSave: "Course de l'adversaire ! Touchez le but pour plonger",
    diveNow: "C'est le moment ! Touchez le but pour plonger",
    shootNow: "C'est le moment ! Touchez le but pour tirer",
    dive: "PLONGEON !",
    reactionLate: "Réaction tardive...",
    shoot: "TIR !",
    timingLate: "Temps tardif...",
    missedFrame: "À CÔTÉ !",
    goalYou: "BUT !!",
    goalCpu: "But encaissé...",
    savedYou: "ARRÊTÉ !",
    savedCpu: "Arrêté par le gardien...",
...
/** キーパーダイブ：初速を速く、伸び切りは滑らかに */
function keeperDiveEase(t) {
  const u = clamp(t, 0, 1);
  return 1 - Math.pow(1 - u, 2.65);
}
...
function keeperBodyCovers(ballAim, diveCell) {
  ...
  if (diveCell.height === "mid") {
    // 中央構え（y: 0.50）：胸元〜腹部（y: 0.30 ~ 0.70）を体でブロック
    if (dy < -0.20) return false;
    if (dy > 0.20) return false;
    return true;
  }
  if (diveCell.height === "high") {
    // ハイダイブ（y: 0.20）：上段をカバー。真ん中〜足元（y >= 0.25）は頭下を抜けて100%ゴール
    if (dy < -0.30) return false;
    if (dy > 0.05) return false;
    return true;
  }
  if (diveCell.height === "low") {
    // ローダイブ（y: 0.68）：下段足元をカバー。真ん中〜上段（y <= 0.63）は頭上を抜けて100%ゴール
    if (dy < -0.05) return false;
    if (dy > 0.30) return false;
    return true;
  }
...
/** ペナルティ地点のボール描画倍率（手前視点） */
const BALL_SPOT_DRAW_RATIO = 1.494;

/** ペナルティ地点のボール描画用 */
function ballSpotDrawRadius(g = goalRect()) {
  const headR = keeperHeadRadius(g);
  return clamp(headR * BALL_SPOT_DRAW_RATIO, 8.1, headR * 1.566);
}
...
function ballBaseRadius(g = goalRect()) {
  const headR = keeperHeadRadius(g);
  return clamp(headR * BALL_SPOT_HEAD_RATIO, 7.2, headR * 1.32);
}
...
function rollStartSpot(isYou, ballX, ballY, w, h, side, kickStyle) {
  const depthMul =
    kickStyle.name === "surge" ? 1.22 : kickStyle.name === "burst" ? 0.72 : kickStyle.name === "snap" ? 0.8 : 1;
  const spots = isYou
    ? [
...
function rollApproach(isYou) {
  ...
  const steps = 3.8 + Math.random() * 1.6 + (kickStyle.runMsMul - 1) * 1.6;
...
function sampleKickerMotion({
  ...
  if (elapsed < runMs) {
    ...
    const burst = u < 0.16 ? Math.pow(u / 0.16, 0.62) * 0.16 : 0;
    let moveEase = burst + (1 - burst) * (1 - Math.pow(1 - u, decelPow));
    if (style.stutter && u > 0.3 && u < 0.48) {
      const st = (u - 0.3) / 0.18;
      moveEase *= 0.92 + 0.08 * Math.sin(st * Math.PI);
    }
    ...
    const strideFreq = (3.15 + runDist / 112) * strideMul * (1 - u * 0.52);
    let stride = (elapsed / 1000) * Math.PI * strideFreq * 2.55;
    if (style.stutter) {
...
/** セーブ時：キーパー（手・腹）での接触点 */
function keeperCatchPoint(result) {
...
/** セーブ後：キーパー正面のゴールライン手前へこぼれる */
function keeperSpillPoint(result, catchPt) {
  const g = goalRect();
  const layout = penaltyLayout();
  const gy = layout.gy;
  const dive = result.dive || { dir: "center", height: "mid" };
  const diveAim = keeperDiveAim(dive.dir, dive.height);
  const keeperX = g.x + diveAim.x * g.w;
  const scatter = (result.saveScatter ?? 0) * g.w * 0.11;
  const lane = layout.spot.y - gy;
  const forward = lane * (0.06 + Math.abs(result.saveScatter ?? 0.35) * 0.09);
  return {
    x: clamp(keeperX + scatter, g.x - g.w * 0.03, g.x + g.w * 1.03),
    y: gy + forward,
  };
}
...
function shotEndPoint(result, ballSpot) {
  ...
  if (result.saved) {
    const catchPt = result.saveCatch ?? keeperCatchPoint(result);
    return keeperSpillPoint(result, catchPt);
  }
...
function flightBallScale(u, result, scaleMul = 1) {
  ...
  const headR = keeperHeadRadius(g);
  // ゴール到達時：見かけ半径＝キーパー頭半径（headR / flightR で一致）
  const goalEnd = clamp(headR / flightR, 0.85, 1.15);
  const goalStart = clamp(goalEnd * 1.25, 1.0, 1.35);
  ...
  if (result?.goal) {
    ...
    end = clamp(goalEnd * 0.92, 0.65, 0.95);
    start = 1.15;
  } else if (result?.post) {
    end = clamp(goalEnd * 1.0, 0.72, 1.0);
    start = 1.15;
  } else {
    end = clamp(goalEnd * 0.92, 0.65, 0.95);
    start = 1.18;
...
function sampleFlightPosition(from, end, postHit, result, u, h) {
  ...
  if (result.saved) {
    const catchPt = result.saveCatch ?? end;
    const spillPt = end;
    const catchU = 0.66;
    const dwellU = 0.07;
    const spillStart = catchU + dwellU;
    ...
    if (u <= catchU) {
      ...
      const arc = h * 0.032;
      return {
        x: lerp(from.x, catchPt.x, ease),
        y: lerp(from.y, catchPt.y, ease) - Math.sin(ease * Math.PI) * arc,
        spinMul: 1,
        scaleMul: 1,
        atImpact: false,
      };
    ...
    if (u <= spillStart) {
      const t = (u - catchU) / dwellU;
      const push = Math.sin(t * Math.PI);
      const dx = catchPt.x - from.x;
      const dy = catchPt.y - from.y;
      const len = Math.hypot(dx, dy) || 1;
      return {
        x: catchPt.x - (dx / len) * push * 3.5,
        y: catchPt.y - (dy / len) * push * 2 + t * h * 0.005,
        spinMul: 0.3,
        scaleMul: 1 - push * 0.07,
        atImpact: t > 0.45,
      };
    }

    const t = (u - spillStart) / (1 - spillStart);
    const drop = 1 - Math.pow(1 - t, 2.35);
    const gravity = t * t * h * 0.024;
    let x = lerp(catchPt.x, spillPt.x, drop);
    let y = lerp(catchPt.y, spillPt.y, drop) + gravity;
    if (t > 0.58) {
      const bt = (t - 0.58) / 0.42;
      y -= Math.sin(bt * Math.PI) * (1 - bt) * h * 0.013;
    }
    return {
      x,
      y,
...
function bindFlightPath(pending, result) {
  ...
  if (result.saved) {
    if (result.saveScatter == null) result.saveScatter = (Math.random() - 0.5) * 1.2;
    result.saveCatch = keeperCatchPoint(result);
  }
...
function stepCpuShot(now) {
  ...
  if (state.diveLocked) {
    const diveT = clamp((elapsed - aimOpen) / (runMs + kickMs * 0.45 - aimOpen), 0, 1);
    if (!state.ball.airborne) state.keeperProgress = keeperDiveEase(diveT) * 0.9;
  }
...
function stepPlayerShot(now) {
  ...
  if (!state.ball.airborne) {
    ...
    if (state.aimLocked && pending.result) {
      const lockAt = pending.aimLockedAt ?? runMs;
      const preFlight = clamp((elapsed - lockAt) / Math.max(kickMs * 0.86, 1), 0, 1);
      state.keeperProgress = keeperDiveEase(preFlight) * 0.8;
    }
  }
...
function drawKeeper() {
  ...
  if (isLowCatch) {
    ...
    const plant = Math.sin(clamp(t / 0.26, 0, 1) * Math.PI) * 1.5 * (1 - stretch);
    const leap = -Math.pow(stretch, 1.55) * 10;
    slideDrop = plant + leap;
  } else if (isSideMidDive) {
    const plantPhase = Math.sin(clamp(t / 0.26, 0, 1) * Math.PI) * (1 - stretch);
  ...
  if (isLowCatch) {
    ...
    // 真横に倒れすぎず、斜め45°前後
    bodyRot = side * tipEase * 0.72;

    // 腰→肩→頭：横へ倒れて腕で届く（体全体は浮かない）
    hipX = side * tipEase * 6;
    hipY = lerp(46, 44, tipEase);
    shoulderX = side * tipEase * 28;
    ...
    bodyRot = side * tipEase * 0.88;

    hipX = side * tipEase * 8;
    hipY = 46 + vertShift;
    shoulderX = side * tipEase * 30;
...
function drawPlayerFigure(opts) {
  ...
  if (pose === "run") {
    ...
    lean = -0.2 - Math.max(0, -s) * 0.07;
    bob = Math.max(0, s) * 2.6;
    hipSwingL = s * 1.05;
    hipSwingR = -s * 1.05;
    kneeL = 0.1 + Math.max(0, -s) * 0.78;
    kneeR = 0.1 + Math.max(0, s) * 0.78;
    plantFootLift = Math.max(0, -s) * 3.2;
    kickFootLift = Math.max(0, s) * 4.8;
    armL = c * 0.54;
    armR = -c * 0.54;
    elbowL = 0.32 + Math.abs(c) * 0.24;
    elbowR = 0.32 + Math.abs(c) * 0.24;
    torsoTwist = s * 0.09;
  } else if (pose === "plant") {
